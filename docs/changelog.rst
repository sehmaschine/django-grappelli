...
* Fixed: changelist pagination numbers rendered as oversized filled buttons with unreadable labels, and the current page was no longer highlighted. Django now emits paginator numbers as ``<a role="button">`` and the current page as ``<a aria-current="page">`` (previously ``<span class="this-page">``). The ``a[role="button"]`` button styling added for the password-reset link (#1073) was catching the former; it is now scoped to ``a[role="button"].button``. The current-page style is now keyed off ``a[aria-current="page"]`` as well as ``span`` (#1080).
* Fixed: on a filtered changelist, the paginator carried a reset link (``href="?"``) next to the "Show all" link, and with ``show_full_result_count = False`` both of them were labelled "Show all". Clicking the reset link dropped the filters, and ``_popup``/``_to_field`` with them in a related-lookup popup. The paginator now states the counts as plain text and keeps a single link, "Show all", which puts every row on one page with the filters intact. Clearing filters is an explicit "Clear all filters" link in the filter pulldown, built from ``cl.has_active_filters`` and ``cl.clear_all_filters_qs``, so it removes the filter parameters only and leaves the search query in place (#1087).
