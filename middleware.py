...
class JavaScript404Patch():
    ...
    def __init__(self):
        self.strip = ['actions','SelectBox','SelectFilter2','calendar',
                      'DateTimeShortcuts', 'CollapsibleGroup']
...
# instead of the user agent string alone, but meanwhile that works fine.
#
...
class BrowserCompatibilityEnforcer():
    ...
    def __init__(self):
        ...
        self.banned_keywords  = getattr(settings, 'BANNED_BROWSERS',  ['Mozilla/4.0', 'MSIE'])
        self.allowed_keywords = getattr(settings, 'ALLOWED_BROWSERS', ['Mozilla/5.0', 'Gecko', 'AppleWebKit', 'Chrome', 'Safari'])
    ...
    def process_response(self, request, response):
        ...
        if not valid and 'noua' not in request.GET:
            ...
            response.content = t.render(Context({'title': settings.ADMIN_TITLE, 'adminmedia': settings.ADMIN_MEDIA_PREFIX}))
        return response
