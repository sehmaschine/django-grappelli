...
(function (window, document, $) {
	...
	$.extend(F, {
		...
		_afterZoomIn: function () {
			...
			if (current.closeBtn) {
				$(F.current.tpl.closeBtn).appendTo($("div#fancybox-wrapper")).bind('click.fb', F.close);
			}

			//Create navigation arrows
			if (current.arrows && F.group.length > 1) {
				if (current.loop || current.index > 0) {
					$(current.tpl.prev).appendTo($("div#fancybox-wrapper")).bind('click.fb', F.prev);
				}

				if (current.loop || current.index < F.group.length - 1) {
					$(current.tpl.next).appendTo($("div#fancybox-wrapper")).bind('click.fb', F.next);
				}
			}
